...
//get frame info
  ...
  h_model = new unsigned char[sizeModel*3];
  model = new pixel[sizeModel];
  ...
  gpuErrchk(cudaMalloc((void**)&d_model,3*sizeModel*sizeof(unsigned char)));
  ...
  while(!frame.empty()){
    ...
    if(frameCount ==0){
      ...
      initialize_model(h_image,model,height,width,N);

      timer1.Stop();
      initialize_time = timer1.Elapsed()/1000;

      unsigned char* temp_model= (unsigned char*)model;
      memcpy(h_model,temp_model,3*sizeModel*sizeof(unsigned char));
     ...
     /* if(verify){
      ...
      timer4.Start();
      gpuErrchk(cudaMemcpy(d_model,h_model,3*sizeModel*sizeof(unsigned char),
              cudaMemcpyHostToDevice));
      timer4.Stop();
      time4 = timer4.Elapsed()/1000;
      memcpy_GPU_TO_CPU += time4;
    ...
    size_t model_memsize = N*3*x_block*y_block*sizeof(unsigned char);
    timer2.Start();
    ...
    timer9.Start();
    cuda_median_blur(d_segmap,height,width,size,memsize,gridSize,blockSize);
    timer9.Stop();
