#include "VIBE-FusedParallelBlur_Overlap-CUDA-kernel.h"
...
//get frame info
  ...
  while(!frame.empty() && frameCount < 400){
    ...
    if(frameCount ==0){
      ...
      timer4.Start();
      if(frameCount ==0){
      gpuErrchk(cudaMemcpy(d_model,h_model,
                sizeModel*sizeof(pixel),cudaMemcpyHostToDevice));
      }
      gpuErrchk(cudaMemcpy(d_segmap,h_segmap,
               sizeRGB*sizeof(unsigned char),
               cudaMemcpyHostToDevice));
      gpuErrchk(cudaMemcpy(d_image,h_image,
               sizeRGB*sizeof(pixel),
               cudaMemcpyHostToDevice));
      timer4.Stop();
    ...
    // segment the frame and time
      ...
      timer9.Start();
      cuda_median_blur(d_segmap,d_blur_map,height,width,size,memsize,
                       gridSize,blockSize);
      timer9.Stop();
      ...
      timer5.Start();                                                                 
      gpuErrchk(cudaMemcpy(blur_map,d_blur_map,
               sizeRGB*sizeof(unsigned char),
