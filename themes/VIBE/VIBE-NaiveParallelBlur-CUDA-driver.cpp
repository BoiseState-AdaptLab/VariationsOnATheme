...
int main(int argc,char* argv[]) {   
  ...
  unsigned char* h_blur= NULL;

  pixel *h_model = NULL;

  pixel *d_image = NULL;
  unsigned char *d_segmap = NULL;
  unsigned char *d_blur = NULL;
  pixel *d_model = NULL;
...
//get frame info
  ...
  while(!frame.empty() && frameCount < 400){
    ...
    gpuErrchk(cudaMalloc((void**)&d_blur,sizeRGB*sizeof(unsigned char)));   
 
    //copy memory from host to device
    timer4.Start();
    gpuErrchk(cudaMemcpy(d_segmap,h_segmap,sizeRGB*sizeof(unsigned char),
              cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_blur,h_segmap,sizeRGB*sizeof(unsigned char),
              cudaMemcpyHostToDevice));
    timer4.Stop();
    ...
    timer7.Start();
    cuda_median_blur(d_segmap,d_blur,height,width,size,memsize,gridSize,blockSize);
    timer7.Stop();
    time7 += timer7.Elapsed()/1000;

    timer5.Start();
    gpuErrchk(cudaMemcpy(h_segmap,d_blur,sizeRGB*sizeof(unsigned char),
