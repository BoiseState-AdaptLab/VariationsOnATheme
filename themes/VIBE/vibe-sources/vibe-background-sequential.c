...
static inline int32_t distance_is_close_8u_C3R(uint8_t r1, uint8_t g1, uint8_t b1, uint8_t r2, uint8_t g2, uint8_t b2, uint32_t threshold,int frame)
{
  if(frame ==2){
  // printf("%d",abs_uint(r1 - r2) + abs_uint(g1 - g2) + abs_uint(b1 - b2));
   //printf("%d,%d,%d,%d,%d,%d",r1,r2,b1,b2,g1,g2);
   //if(abs_uint(r1 - r2) + abs_uint(g1 - g2) + abs_uint(b1 - b2) <= 4.5 * threshold)
    //exit(0);
  }
...
int32_t libvibeModel_Sequential_AllocInit_8u_C3R(
  ...
  {
    uint8_t value = image_data[index];
    if(frame == 1 && index == 3*257*width + 3*226)
    {
     // printf("%d",value);
    }
    for (int x = 0; x < model->numberOfSamples - NUMBER_OF_HISTORY_IMAGES; ++x) {
      int value_plus_noise = value + rand() % 20 - 10;
      if(frame == 1 && index == 3*7*width + 3*266)
      {
        //printf(" %d ",value_plus_noise);
      }
      if (value_plus_noise < 0) { value_plus_noise = 0; }
      if (value_plus_noise > 255) { value_plus_noise = 255; }
...
int32_t libvibeModel_Sequential_Segmentation_8u_C3R(
  ...
  if(frame == 2)
  {
    for(int h = 0;h < 2;h++)
    {
       printf("%d ",first[h*width*height*3 + (256*3*width + 76*3)]);
       printf("%d ",first[h*width*height*3 + (256*3*width + 76*3)+1]);
       printf("%d\n",first[h*width*height*3 + (256*3*width + 76*3)+2]);
      
       
     }

    int ind = 3*(256*width + 76)*18;
    for(int m = 0;m < 18;m++,ind += 3)
    {
       printf("%d ",historyBuffer[ind]);
       printf("%d ",historyBuffer[ind + 1]);
       printf("%d\n",historyBuffer[ind + 2]);
    }
  }
  for (int index = width * height - 1; index >= 0; --index) {
    //if(frame == 2){ index = 269;}  
    if (
      !distance_is_close_8u_C3R( 
        image_data[3 * index], image_data[3 * index + 1], image_data[3 * index + 2],
        first[3 * index], first[3 * index + 1], first[3 * index + 2], matchingThreshold,frame
      )
    ){
  ...
  for (int i = 1; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
    ...
    for (int index = width * height - 1; index >= 0; --index) {
      ...
      if (
        distance_is_close_8u_C3R( 
          image_data[3 * index], image_data[3 * index + 1], image_data[3 * index + 2],
          pels[3 * index], pels[3 * index + 1], pels[3 * index + 2], matchingThreshold,frame
        )
      ){
        if(frame == 2 && index == 5386)
         printf("hi1");
        --segmentation_map[index];}
    }
  }
  ...
  for (int index = width * height - 1; index >= 0; --index) {
    if (segmentation_map[index] > 0) {
     ...
     /* if(frame == 2)
      {
        index =269;
        indexHistoryBuffer = 3*index * numberOfTests + 39;
      }*/
      for (int i = numberOfTests; i > 0; --i, indexHistoryBuffer += 3) {
       /* if(frame == 2)
        {            
         //printf("%d,%d,%d",image_data[(3 * index)], image_data[(3 * index) + 1], image_data[(3 * index) + 2]);
         printf("%d,%d,%d",historyBuffer[indexHistoryBuffer], historyBuffer[indexHistoryBuffer + 1], historyBuffer[indexHistoryBuffer + 2]);
        // exit(0);
        }*/
        ...
        if (
          distance_is_close_8u_C3R( 
            ...
            matchingThreshold,frame
          )
        ){
          if(frame ==2 && index == 5386){
          // printf("%d",segmentation_map[index]);
  ...
  for (uint8_t *mask = segmentation_map; mask < segmentation_map + (width * height); ++mask)
  {
   if(mask == segmentation_map + 8*width + 266 && frame == 2)
   {
      printf("RE%d",*mask);
   } 
    if (*mask > 0) *mask = COLOR_FOREGROUND;
