...
__global__ void RGBtoGRAY(const uchar* const InputRGB,
 ...
 for(k = 0;k < 72 ;k++)
   ...
   if(i < cols)
    {
     idx = (j*cols*72 + i + k*cols);

      OutputGRAY[idx] = (uchar)((look_up_table[InputRGB[(idx)*3]]
                + look_up_table[InputRGB[( idx)*3+1]+256] + look_up_table[InputRGB[(idx)*3+2]+512]) >> yuv_shift);
   }
 }
...
*
******************************************************************************/
int  test_OpenCV_int(const unsigned char *Image1,const uchar *gray,const int* const look_tab,int rows,int cols,int frame,int len)
{
  //printf("inside4");
...
gpuErrchk(cudaMemset(d_imageGRAY,0,sizeof(unsigned char)*numPixels));
...
const dim3 gridSize(ceil((ImageRGB.cols*1.0)/x_block),10,1);
...
// Reading the frame
    ...
    while(cap.isOpened() && frameCount == 1)
     {
       ...
       if(!frame.empty())
        ...
        allocate(ImageRGB,h_imageRGB,h_imageGRAY,table,numPixels,frameLength,frameCount,v,x,y);
 ...
 if(verify == true)
 { 
   frameCount = test_OpenCV_int(h_imageRGB,h_imageGRAY,table,ImageRGB.rows,ImageRGB.cols,frameCount,frameLength);
 }
