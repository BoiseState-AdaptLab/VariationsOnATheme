...
int main(int argc, char **argv)
{
  ...
  /* Allocates the model and initialize it with the first image. */
  libvibeModel_Sequential_AllocInit_8u_C3R(model, image_data, width, height,height);
  ...
  while (!finished(stream)) {
    ...
    /* Segmentation step: produces the output mask. */
    libvibeModel_Sequential_Segmentation_8u_C3R(model, image_data, segmentation_map,height);
