...
double run_schedule(const string& schedule, real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int its, int ots);

// #1
void schedule_baseline(real *data, int ubT, int lb1, int ub1, int lb2, int ub2);
// #2
void schedule_unrolled(real *data, int ubT, int lb1, int ub1, int lb2, int ub2);
// #3
void schedule_fuse_0_0_1_1_( real *data, int ubT, int lb1, int ub1, int lb2, int ub2);
// #4
void schedule_tile_inner_inner_serial_serial_fuse_0_0_1_1_(real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int its);
// #5
void schedule_fuse_0_0_1_1_tile_inner_inner_serial_serial_( real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int its );
// #6
void schedule_tile_inner_inner_serial_tile_outer_outer_fuse_0_0_1_1_( real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int its, int ots );
// #7
void schedule_tile_inner_inner_tile_outer_outer_serial_fuse_0_0_1_1_( real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int its, int ots );
// #8
void schedule_tile_inner_inner_tile_10_10_parallel_serial_serial_( real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int its, int ots );
// #9
void schedule_tile_outer_outer_tile_parallel_serial_( real *data, int ubT, int lb1, int ub1, int lb2, int ub2, int ots );
...
int main( int argc, char* argv[] ){
  ...
  **************************************************************************/
  ...
  int i;
  int j;
  Nx = configuration.getInt("Nx");
  Ny = configuration.getInt("Ny");
...
// 0 This schedule is an implementiation that looks like what you would 
// typically see in Chombo code
void schedule_loopchain(real *data, int ubT, int lb1, int ub1, int lb2, int ub2) {
    int i, j;
    ...
    for( int t = 1; t <= ubT; ++t ){
        ...
        for( i = lb1; i <= ub1; ++i ){
            for( j = lb2; j <= ub2; ++j ){
                /*space(data,(t),(i),(j)) =                          \
                                        (space(data, (t)-1, (i)  , (j)  )   + \
                                         space(data, (t)-1, (i)  , (j)+1)   + \
                                         space(data, (t)-1, (i)  , (j)-1)   + \
                                         space(data, (t)-1, (i)+1, (j)  )   + \
                                        space(data, (t)-1, (i)-1, (j)  ) );*/
                space(data,(t),(i),(j)) =                          \
                                        (space(data, (t)-1, (i)  , (j)  )   + \
                                         space(data, (t)-1, (i)  , (j)+1)   + \
                                         space(data, (t)-1, (i)  , (j)-1));
                space(data,t,i,j) +=    (space(data, (t)-1, (i)+1, (j)  )   + \
                                        space(data, (t)-1, (i)-1, (j)  ) );
            }
        }
        //for( i = lb1; i <= ub1; ++i ){
...
for( int t = 1; t <= ubT; t += 2 ){
  for (int c2 = lb2; c2 <= ub2; c2 += 1)
    stencil(data,t,lb1, c2);
  for (int c1 = lb1 + 1; c1 <= ub1; c1 += 1) {
    stencil(data,t, c1, lb2);
    for (int c2 = lb2 + 1; c2 <= ub2; c2 += 1) {
      stencil(data,t, c1, c2);
      stencil(data,t+1, c1 - 1, c2 - 1);
    }
    stencil(data,t+1,c1 - 1, ub2);
  }
  for (int c2 = lb2 + 1; c2 <= ub2 + 1; c2 += 1)
    stencil(data,t+1,ub1, c2 - 1);
}}
}
// 4  ==========================================================
