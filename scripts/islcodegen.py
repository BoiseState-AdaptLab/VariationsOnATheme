...
def main():
    ...
    # Change working directory before compiling to ensure relative paths are correct!
    ...
    compile = '%s %s %s %s %s -o %s' % (build['CXX'], build['CFILES'], benchmark['main'],
                                        build['CFLAGS'], build['COPTS'], benchmark['exec'])
    if len(args.build) > 0:
